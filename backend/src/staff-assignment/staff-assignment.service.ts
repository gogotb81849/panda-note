...
export class StaffAssignmentService {
  ...
  async initializeFromShips(teamCode: string, userId: number = 0) {
    ...
    // 预查所有政委用户ID，避免外键约束失败
    const officerIds = ships.map(s => s.politicalOfficerId).filter(Boolean) as number[];
    ...
    const invalidUserShips: { shipName: string; officerId: number; officerName: string }[] = [];

    for (const ship of ships) {
      if (!ship.politicalOfficerId) continue;

      // 校验政委用户是否存在（避免外键约束失败）
      if (!validUserIds.has(ship.politicalOfficerId)) {
        invalidUserCount++;
        invalidUserShips.push({
          shipName: ship.cnShipName,
          officerId: ship.politicalOfficerId,
          officerName: ship.politicalOfficerName || '',
        });
        continue;
      }
    ...
    await this.operationLogService.create({
      ...
      teamCode,
      operationType: '新增',
      operationContent: `初始化派任数据：新增${createdCount}条，跳过${skippedCount}条（已有记录），无效政委${invalidUserCount}条`,
    });
