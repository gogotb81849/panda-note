...
#
# 只做一件事：
#   P1. 熊猫笔记自修复（PM2 状态检查 + 双进程 restart 确认）
#
# ★ 2026-08-16 陈先生要求：海上菜篮子项目暂时断开（不再联动部署）
#   原来的 P2（联动更新海上菜篮子）已整段注释掉（搜索"陈先生断开菜篮子 P2 注释起点"可找到）
# ============================================================
...
SHIP_TARGET_TAG="$2"         # 保留向后兼容（2026-08-16 陈先生断开菜篮子后，不会再使用）
LOG_FILE="/var/log/panda-note-server-side-deploy.log"
mkdir -p "$(dirname "$LOG_FILE")"
exec > >(tee -a "$LOG_FILE") 2>&1

log()  { echo "[$(date '+%Y-%m-%d %H:%M:%S')][panda-server-deploy] $*"; }
die()  { log "safe-skip: $* (deploy succeeded anyway)"; exit 0; }

log "============================================================"
log "参数: PANDA_VERSION=$PANDA_VERSION SHIP_TARGET_TAG=$SHIP_TARGET_TAG（2026-08-16 陈先生断开菜篮子，SHIP_TARGET_TAG 不再使用）"
...
# // ====== 陈先生断开菜篮子 P2 注释起点（2026-08-16，运行成功后可删除）=====================
# # ---------- P2. 联动更新海上菜篮子 ----------
...
log "Ship Plant version check skipped（2026-08-16 陈先生断开菜篮子）"
